...
class SoalScreen(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.judul_entry = ctk.CTkEntry(self, width=300, placeholder_text="Contoh: Materi 1 - Percabangan")
        self.judul_entry.place(relx=0.3, rely=0.8, anchor="w")

        ctk.CTkLabel(self, text="Deskripsi:").place(relx=0.2, rely=0.85, anchor="w")
        self.deskripsi_entry = ctk.CTkEntry(self, width=300, placeholder_text="Deskripsi singkat materi")
        ...
        ctk.CTkButton(self, text="Tambah / Update Materi", command=self.save_materi).place(
            relx=0.6, rely=0.8, relwidth=0.15, relheight=0.07)
        ctk.CTkButton(self, text="Hapus Materi", command=self.delete_materi).place(
            relx=0.8, rely=0.8, relwidth=0.15, relheight=0.07)
        ctk.CTkButton(self, text="Kembali", command=self.back_to_dashboard).place(
            relx=0.9, rely=0.95, relwidth=0.08, relheight=0.05)
    ...
    def select_materi(self, id_materi, judul, deskripsi):
        ...
        self.status_label.configure(text=f"Mengedit Materi ID: {id_materi}", text_color="orange")

    def save_materi(self):
        judul = self.judul_entry.get().strip()
        deskripsi = self.deskripsi_entry.get().strip()
        ...
        try:
            ...
            if self.selected_id:
                ...
                self.status_label.configure(text="Materi berhasil diperbarui.", text_color="green")
            else:
                cursor.execute("INSERT INTO materi (judul, deskripsi) VALUES (?, ?)", (judul, deskripsi))
                self.status_label.configure(text="Materi berhasil ditambahkan.", text_color="green")

            conn.commit()
        except Exception as e:
            self.status_label.configure(text="Gagal menyimpan materi!", text_color="red")
            print("DB Error:", e)
        finally:
            conn.close()
    ...
    def delete_materi(self):
        if not self.selected_id:
            self.status_label.configure(text="Pilih materi yang ingin dihapus!", text_color="red")
            return
        ...
        try:
            ...
            conn.commit()
            self.status_label.configure(text="Materi berhasil dihapus.", text_color="green")
        except Exception as e:
            self.status_label.configure(text="Gagal menghapus materi!", text_color="red")
            print("DB Error:", e)
        finally:
            conn.close()
